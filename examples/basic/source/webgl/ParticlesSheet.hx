...
/**
 * Shows a simple particle emitter with sprite sheet for each particle.
 * @author Eduardo Pons - eduardo@thelaborat.org
 */
class ParticlesSheet extends Application implements IUpdateable
{
	...
	{		
		...
		//Loads the ImageElement 'particle_explosion' and stores it as a TextureHTML in 'particle'
		Asset.LoadTextureHTML("particle", "http://haxor.thelaborat.org/resources/texture/particle/sheet/explosion_additive.jpg");
	...
	{
		trace("ParticlesSheet> Initialize");
		...
		particle.life.size.start = new Vector3(0.5,0.5,0.5);
		...
		particle.start.life.start    = 0.9;
		particle.start.life.end      = 0.9;
		...
		particle.start.speed.random   = false;
		...
		var ps : ParticleSheet = particle.sheet;
		
		
		ps.length = 25;					//25 frames
		ps.fps    = 25;					//10 FPS animation
		ps.width  = tex.width / 5.0;    //Size of the frame cell width
		ps.height = tex.height / 5.0;   //Size of the frame cell height
		ps.wrap   = AnimationWrap.Clamp;//Animation must end and stay in the last frame.
		
		//Yes you can create textures and manipulate pixels :)
		var color_fade : Texture2D = new Texture2D(4, 1, TextureFormat.RGBA8);
		
		//Sets the pixels
		color_fade.SetPixels([Color.empty, Color.white, Color.white, Color.white]);
		...
		mat.SetUniform("Texture", tex); 			//Sets the 'DiffuseTexture' Uniform to the loaded texture.
