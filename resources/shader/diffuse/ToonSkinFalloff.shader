<shader id="haxor/diffuse/ToonSkinFalloff">	
		...
		<fragment precision="medium">
			...
			{ 
				...
				if(p_id==4) return Lights[12+1].xyz;
				if(p_id==5) return Lights[15+1].xyz;
				if(p_id==6) return Lights[18+1].xyz;
				if(p_id==7) return Lights[21+1].xyz;
				return vec3(0,0,0);
			}
			...
			{ 
				...
				if(p_id==4) return Lights[12+2];
				if(p_id==5) return Lights[15+2];
				if(p_id==6) return Lights[18+2];
				if(p_id==7) return Lights[21+2];
				return vec4(0,0,0,1);
			}
			...
			{				
				...
				vec3  df = texture2D(RampTexture,vec2(d,0.0)).xyz;
				vec3  c  = p_diffuse.xyz * p_color.xyz * df * p_intensity;
				return c;
			}
			...
			{	
				...
				float falloff_factor = (1.0 - clamp(dot(normalize(v_wsView), n),0.0,1.0));
				
				
				c += tex_diffuse.xyz * pow(falloff_factor,Falloff) * FalloffIntensity;
